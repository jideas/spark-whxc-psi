 ...
 * 入库单明细列表处理器
 */
public class InspectCheckingInDetailProcessor extends ExtendSimpleSheetPageProcessor<CheckingGoodsItem> implements
		IDataProcessPrompt {
	...
	public static enum Columns {
		GoodsCode, GoodsName, GoodsProperties, GoodsUnit, CheckingCount, CheckedCount, CheckCount, GoodsNo, InspectCount
	}

	private Button checkIn, inspectButton;
	...
	private void initContorls() {
		...
		inspectButton = new Button(this.createControl(ID_RenderButtonArea, Composite.class), JWT.APPEARANCE3);
		inspectButton.setID(ID_Button_Inspect);
		inspectButton.setText("材料拒收");
	 ...
	 * 材料待检
	 */
	protected void goodsInspect(boolean isCheckIn) {
		if (!validateInput()) {
			return;
		}
		...
		InspectCheckinTask task = new InspectCheckinTask(false);
		...
		for (int i = 0; i < rowIds.length; i++) {
			...
			if (CheckIsNull.isNotEmpty(values[0]) && Double.valueOf(values[0]) > 0) {
				double value = DoubleUtil.strToDouble(values[0]);
				double oldValue = map.get(GUID.valueOf(rowId)).getInspectCount();
				if (value > oldValue) {
					alert("入库数量不能大于待检数量！");
					return;
				}
				itemList.add(task.new InspectItem(GUID.valueOf(rowId), value, oldValue));
			}
		}
