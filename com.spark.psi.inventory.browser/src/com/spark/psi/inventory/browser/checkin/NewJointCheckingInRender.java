...
import com.spark.psi.inventory.browser.checkin.NewJointCheckingInProcessor.Column;
import com.spark.psi.publish.base.materials.entity.MaterialsItemInfo;

public class NewJointCheckingInRender extends PSIListPageRender {

	@Override
	protected void afterFooterRender() {
		super.afterFooterRender();
		Button confirmBtn = new Button(footerRightArea, JWT.APPEARANCE3);
		confirmBtn.setID(NewJointCheckingInProcessor.ID_Button_Confirm);
	...
	protected void afterTableRender() {
		...
		addGoodsBtn.setID(NewJointCheckingInProcessor.ID_Button_AddGoods);
		...
		memoText.setID(NewJointCheckingInProcessor.ID_Text_Memo);
	}
	...
	protected void beforeTableRender() {
		...
		storeList.setID(NewJointCheckingInProcessor.ID_List_Store);
	...
	public STableColumn[] getColumns() {
		...
		columns[0] = new STableColumn(NewJointCheckingInProcessor.Column.code.name(), 100, JWT.LEFT,
				NewJointCheckingInProcessor.Column.code.getTitle());
		columns[1] = new STableColumn(NewJointCheckingInProcessor.Column.number.name(), 100, JWT.LEFT,
				NewJointCheckingInProcessor.Column.number.getTitle());
		columns[2] = new STableColumn(NewJointCheckingInProcessor.Column.name.name(), 100, JWT.LEFT,
				NewJointCheckingInProcessor.Column.name.getTitle());
		columns[3] = new STableColumn(NewJointCheckingInProcessor.Column.property.name(), 100, JWT.LEFT,
				NewJointCheckingInProcessor.Column.property.getTitle());
		columns[4] = new STableColumn(NewJointCheckingInProcessor.Column.unit.name(), 100, JWT.CENTER,
				NewJointCheckingInProcessor.Column.unit.getTitle());
		columns[5] = new SNumberEditColumn(NewJointCheckingInProcessor.Column.count.name(), 100, JWT.RIGHT,
				NewJointCheckingInProcessor.Column.count.getTitle());
