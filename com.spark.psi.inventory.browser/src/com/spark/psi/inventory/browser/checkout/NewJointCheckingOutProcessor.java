...
public class NewJointCheckingOutProcessor extends PSIListPageProcessor<CheckingInItem> implements IDataProcessPrompt {
	...
	public static enum Column {
		code("材料编号"), number("材料条码"), name("材料名称"), property("规格"), unit("单位"), count("数量"), inventoryCount("库存数量");
	...
	public void process(final Situation context) {
		...
		addGoodsBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				MsgRequest request = CommonSelectRequest.createSelectMaterialsRequest(GUID.valueOf(storeId), true);
	...
	private boolean doSave() {
		...
		final JointCheckoutTask task = new JointCheckoutTask();
		if (!validateInput()) {
			return false;
		}
		...
		for (JointCheckoutTaskItem item : task.getItems()) {
	...
	private boolean fillDataToTask(JointCheckoutTask task) {
		...
		List<JointCheckoutTaskItem> items = new ArrayList<JointCheckoutTaskItem>();
		for (String rowId : rowIds) {
			...
			items.add(new JointCheckoutTaskItem(goodsId, count));
		}
		task.setItems(items);
		return true;
	}
