 ...
 * 已处理完成的出库单列表视图
 */
public class ApprovalGoodsSplitListRender extends PSIListPageRender {
	...
	protected void afterFooterRender() {
		...
		new SSearchText2(headerRightArea)
				.setID(ApprovalGoodsSplitListProcessor.ID_TEXT_SEARCH);
		// new
		// Button(headerRightArea).setID(NewGoodsSplitListProcessor.ID_BUTTON_SEARCH);
		//
		new LWComboList(headerLeftArea, JWT.APPEARANCE3)
				.setID(ApprovalGoodsSplitListProcessor.ID_COMBOLIST_DATEITEM);
		new Label(headerLeftArea).setText("  单据数量：");
		new Label(headerLeftArea)
				.setID(ApprovalGoodsSplitListProcessor.ID_LABEL_CHECKOUTGINSHEET_COUNT);
	}
