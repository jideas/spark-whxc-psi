 ...
 * 已处理完成的出库单列表视图
 */
public class ProcessingGoodsSplitListRender extends PSIListPageRender {
	...
	protected void afterFooterRender() {
		...
		//
		new SSearchText2(headerRightArea).setID(ProcessingGoodsSplitListProcessor.ID_TEXT_SEARCH);
//		new Button(headerRightArea).setID(NewGoodsSplitListProcessor.ID_BUTTON_SEARCH);
		//
		new LWComboList(headerLeftArea,JWT.APPEARANCE3).setID(ProcessingGoodsSplitListProcessor.ID_COMBOLIST_DATEITEM);		
		new Label(headerLeftArea).setText("  单据数量：");
		new Label(headerLeftArea).setID(ProcessingGoodsSplitListProcessor.ID_LABEL_CHECKOUTGINSHEET_COUNT);
	}
