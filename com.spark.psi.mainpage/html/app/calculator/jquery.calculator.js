...
function Calculator() {
	...
	this._keyDefs = {
		...
		'_=': ['=', this.unary, this._equals, 'arith equals', 'EQUALS', 'Ent'],
		...
		'CA': ['C', this.control, this._clear, 'clear', 'CLEAR', 35, 'End'], // end
		'@X': ['#close', this.control, this._close, 'close', 'CLOSE', 27, 'Esc'], // escape
		'@U': ['=', this.control, this._use, 'use', 'USE', 13, 'Ent'], // enter
...
$.extend(Calculator.prototype, {
	...
	_attachCalculator: function(target, settings) {
		...
		var keyEntry = (!inline ? $target :
			$('<input style="opacity: 0;filter:alpha(opacity=0);" type="text" class="' + this._inlineEntryClass + '"/>'));
	...
	_generateHTML: function(inst) {
		...
		var html = (!prompt ? '' : '<div class="calculator-prompt' +
			...
			(inst._focussed ? ' ' + this._focussedClass: '') + '" ><span><font class="span_font">' +
			inst.dispValue + '</font></span></div>';
		for (var i = 0; i < layout.length; i++) {
			html += '<div class="calculator-row" style="white-space:nowrap;overflow:hidden;">';
			for (var j = 0; j < layout[i].length; j += 2) {
				...
				if(label == '0') {
					html += "<div>";
					html += '<table height="20px" width="72px" border="0" cellpadding= "0" cellspacing= "0" class="tableca" >';
					html += '<tr class="tr1" height="1px" >';
					html += '<td class="button-top-left" width="10px"></td>';
					html += '<td class="button-top-center" width="64px" height="3px"></td>';
					html += '<td class="button-top-right" width="2px"></td>';
					html += '</tr>';
					
					html += '<tr class="tr2">';
					html += '<td class="button-center-left" width="2px"></td>';
					html += '<td class="button-center-center" height="18px" width="64px">';
					...
					' disabled="disabled"' : '') + ' class="calculator-digit2' :
					// Binary operations
					...
					(status ? 'title="' + status + '"' : '') + 'style="background-color:transparent;height:18px;width:64px;border:0"><font style="color:#FFFFFF">' +
					(code == '_.' ? this._get(inst, 'decimalChar') : label) +
					// Keystrokes
					(def[5] && def[5] != def[0] ?
					'</font><span class="' + this._keystrokeClass + (useTR ? ' ui-state-error' : '') +
					...
					html += '<td class="button-bottom-center" width="64px"></td>';
					...
					html += "<div>";
					html += '<table height="20px" width="33px" border="0" cellpadding= "0" cellspacing= "0" class="tableca">';
					html += '<tr class="tr1" height="1px" >';
					html += '<td class="button-top-left" width="10px"></td>';
					html += '<td class="button-top-center" width="25px" height="3px"></td>';
					html += '<td class="button-top-right" width="2px"></td>';
					html += '</tr>';
					
					html += '<tr class="tr2">';
					html += '<td class="button-center-left" width="2px"></td>';
					html += '<td class="button-center-center" height="18px" width="25px">';
					html += (def[1] == this.space ? '<span class="calculator-' + def[3] + '"></span>' :
					(inst._inline && (def[2] == '._close' || def[2] == '._erase') ? '' :
					'<button type="button"  keystroke="' + code + '"' +
					// Control buttons
					(def[1] == this.control ? ' class="calculator-ctrl2' +
					(def[0].match(/^#base/) ? (def[0].replace(/^#base/, '') == base ?
					uiActive || ' calculator-base-active' : uiHighlight) :
					(def[0] == '#degrees' ? (useDegrees ? uiActive || ' calculator-angle-active' : uiHighlight) :
					(def[0] == '#radians' ? (!useDegrees ? uiActive || ' calculator-angle-active' : uiHighlight) :
					...
					// Common
					(useTR ? ' ui-state-default' : '') + (styles ? ' ' + styles : '') + '" ' +
					(status ? 'title="' + status + '"' : '') + 'style="background-color:transparent;height:18px;width:25px;border:0"><font style="color:#FFFFFF">' +
					(code == '_.' ? this._get(inst, 'decimalChar') : label) +
					// Keystrokes
					(def[5] && def[5] != def[0] ?
					'</font><span class="' + this._keystrokeClass + (useTR ? ' ui-state-error' : '') +
					...
					html += '<tr class="tr3" height="1px" >';
					html += '<td class="button-bottom-left" width="2px"></td>';
					html += '<td class="button-bottom-center" width="25px"></td>';
					html += '<td class="button-bottom-right" width="2px"></td>';
					html += '</tr>';
					html += '</table>';
					html += '</div>';
		...
		html = $(html);
		html.find('button').mouseover(function() { 
