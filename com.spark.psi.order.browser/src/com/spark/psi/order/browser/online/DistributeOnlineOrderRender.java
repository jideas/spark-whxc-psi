...
public class DistributeOnlineOrderRender extends BaseFormPageRender {
	/**
	 * 是否自定义Form布局
	 * 
	 * @return
	...
	private void renderLeftArea(Composite parent) {
		...
		label.setText("选择仓库：");
		label.setID(DistributeOnlineOrderProcessor.ID_Label_StoreName);
//		label.setID(DistributeSalesOrderProcessor.ID_Label_StoreInfo);
		...
		//
		STableColumn column = new STableColumn(DistributeOnlineOrderProcessor.StoreTableColumn.name.name(), 100, JWT.LEFT, "仓库名称");
		column.setGrab(true);
		...
		STable table = new STable(parent, new STableColumn[] { column },
				tableStyle);
		table.setID(DistributeOnlineOrderProcessor.ID_Table_Store);
	...
	private void renderRightArea(Composite parent) {
		...
		label = new Label(header);
		label.setID(DistributeOnlineOrderProcessor.ID_Button_ResolveAll);
		...
		columns[0] = new STableColumn(DistributeOnlineOrderProcessor.MaterialTableColumn.name.name(), 200, JWT.LEFT, "材料名称");
		columns[1] = new STableColumn(DistributeOnlineOrderProcessor.MaterialTableColumn.unit.name(), 100, JWT.CENTER, "单位");
		columns[2] = new STableColumn(DistributeOnlineOrderProcessor.MaterialTableColumn.orderCount.name(), 130, JWT.RIGHT, "订货数量");
		columns[3] = new STableColumn(DistributeOnlineOrderProcessor.MaterialTableColumn.UnAllocate.name(), 130, JWT.RIGHT, "未分配量");
		columns[4] = new STableColumn(DistributeOnlineOrderProcessor.MaterialTableColumn.Available.name(), 130, JWT.RIGHT, "本库可用");
		columns[5] = new SNumberEditColumn(DistributeOnlineOrderProcessor.MaterialTableColumn.Allocate.name(), 130, JWT.RIGHT, "本库分配");
		...
		SEditTable table = new SEditTable(parent, columns, tableStyle,
				null);
		table.setID(DistributeOnlineOrderProcessor.ID_Table_Material);
		table.setLayoutData(GridData.INS_FILL_BOTH);
	}
	...
	private void renderBottomArea(Composite bottomArea) {
		...
		memoText.setID(DistributeOnlineOrderProcessor.ID_Text_Remark);
	...
	protected void renderButton(Composite buttonArea) {
		...
		button.setText(" 重新分配 ");
		button.setID(DistributeOnlineOrderProcessor.ID_Button_Reset);
		button = new Button(buttonArea, JWT.APPEARANCE3);
		button.setText(" 生成生产订单 ");
		button.setID(DistributeOnlineOrderProcessor.ID_Button_Confirm);
	}
}
