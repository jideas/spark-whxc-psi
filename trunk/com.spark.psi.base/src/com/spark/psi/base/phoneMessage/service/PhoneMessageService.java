...
public class PhoneMessageService extends Service {
	 ...
	 */
	@Publish
	protected class MsgAccountBalanceProvider extends OneKeyResultProvider<MsgAccountBalance, GetMessageBalanceKey> {

		@Override
		protected MsgAccountBalance provide(Context context, GetMessageBalanceKey key) throws Throwable {
			if (cannotSendMsg()) {
				return new MsgAccountBalance("该时间段内禁止操作！");
			}
			String parameter = "";
			if (CheckIsNull.isNotEmpty(key.getCompAccountKey())) {
				parameter = key.getCompAccountKey() + "=" + key.getCompAccount() + "&" + key.getUserNameKey() + "="
						+ key.getUserName() + "&" + key.getPasswordKey() + "="
						+ EncryptionUtil.encryptMD5(key.getPassword()).toLowerCase();
			} else {
				parameter = key.getUserNameKey() + "=" + key.getUserName() + "&" + key.getPasswordKey() + "="
			...
			// sb.toString().substring(sb.toString().indexOf("#") + 1) + "条。");
	 ...
	 */
	@Publish
	protected class PhoneMessageSendHandle extends SimpleTaskMethodHandler<PhoneMessageSendTask> {
		...
		@Override
		protected void handle(Context context, PhoneMessageSendTask task) throws Throwable {
			...
			if (task.getFlag() == ReturnFlag.Success) {
				context.asyncHandle(new PhoneMessageLogTask(task.getPhoneNo(), task.getSendType()));
			}
			// if (sb.toString().startsWith("1")) {
	 ...
	 */
	@Publish
	protected class WrightMessageLogHandle extends SimpleTaskMethodHandler<PhoneMessageLogTask> {
		@Override
		protected void handle(Context context, PhoneMessageLogTask task) throws Throwable {
			...
			ib.addColumn("remoteAddress", ib.STRING, context.getLogin().getRemoteInfo().getRemoteAddr());
			ib.addColumn("remoteHost", ib.STRING, context.getLogin().getRemoteInfo().getRemoteHost());
