...
public class MaterialInventoryInfoProcessor extends PageProcessor {
	...
	private MaterialsInfo goodsInfo;
	
	private Composite contentArea;
	private LWComboList listScope;
	private LWComboList listType;
	
	private List<MaterialItemInventoryInfoPage> itemInventoryList = new ArrayList<MaterialItemInventoryInfoPage>();
	...
	public void init(Situation context) {
		if(getArgument() != null && getArgument() instanceof GUID) {
			...
			goodsInfo = getContext().find(MaterialsInfo.class, goodsId);
		}
...
//		goodsInfo.
		...
		if (InventoryWarningType.ALL_Amount.equals(goodsInfo.getMaterialsWarnningType())) {
			viewScope = GoodsInventoryScope.Total;
			viewType = ThresholdControlType.AMOUNT;
		} else if (InventoryWarningType.ALL_Count.equals(goodsInfo.getMaterialsWarnningType())) {
			viewScope = GoodsInventoryScope.Total;
			viewType = ThresholdControlType.COUNT;
		} else if (InventoryWarningType.Store_Amount.equals(goodsInfo.getMaterialsWarnningType())) {
			viewScope = GoodsInventoryScope.EachStore;
			viewType = ThresholdControlType.AMOUNT;
		} else if (InventoryWarningType.Store_Count.equals(goodsInfo.getMaterialsWarnningType())) {
			viewScope = GoodsInventoryScope.EachStore;
			viewType = ThresholdControlType.COUNT;
		}
//		try {
...
//				getContext().broadcastMessage(new GoodsItemLimitMsg());
				...
				for(MaterialItemInventoryInfoPage itemPage : itemInventoryList) {
...
//		listScope.setSelection(viewScope == null ? GoodsInventoryScope.Total.name() : viewScope);
		...
		for(MaterialsItemData goodsItem : goodsInfo.getItems()) {
			MaterialItemInventoryInfoPage itemInventoryPage = new MaterialItemInventoryInfoPage(contentArea, goodsItem, viewScope, viewType, isShowOnly);
