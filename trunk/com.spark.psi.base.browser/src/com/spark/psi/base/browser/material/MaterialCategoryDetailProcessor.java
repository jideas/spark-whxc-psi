...
public class MaterialCategoryDetailProcessor extends PageProcessor implements IDataProcessPrompt{
...
//	private ComboList decimalList;
	...
	final static String[] DEFAULT_UNITS = new String[] {
			"份", "公斤", "斤", "箱", "瓶", "包", "升" };

	private String lastSelectedPropertyRowId;
	// 临时存储界面上的属性定义值
	private final Map<String, MaterialsPropertyDefine> ui_propertyDefines = new HashMap<String, MaterialsPropertyDefine>();
	...
	private MaterialsCategoryInfo categoryInfo;
	private MaterialsCategoryInfo propertiedParentNode = null;
	...
	public void init(Situation context) {
		...
		categoryInfo = context.find(
				MaterialsCategoryInfo.class, categoryId);

		final GUID propertiedParentNodeId = argument[1] == null ? null
				: argument[1];
		if (null != propertiedParentNodeId) {
			propertiedParentNode = context.find(MaterialsCategoryInfo.class,
					propertiedParentNodeId);
		}
	}
	@Override
	public void process(Situation context) {
		categoryNameLabel = createLabelControl(ID_Label_CategoryName);
//		decimalList = createControl(ID_ComboList_Decimal, ComboList.class);
		...
		context.regMessageListener(MaterialCategorySelectionMsg.class, new MessageListener<MaterialCategorySelectionMsg>() {
			...
			public void onMessage(Situation context,
				...
				categoryInfo = context.find(
						MaterialsCategoryInfo.class, categoryInfo.getId());
...
//		}
	...
	private void initPropertyDataShow() {
		//
		MaterialsPropertyDefine[] propertyDefines;
...
//					"规格", null);
			propertyDefines = new MaterialsPropertyDefine[] { unitItem }; // TOOD:单位枚举
			// 默认选择单位（第一行）
			lastSelectedPropertyRowId = unitItem.getId().toString(); 
		}
		for (MaterialsPropertyDefine pd : propertyDefines) {
			if (pd.getId() == null) {
		...
		//
		if (propertiedParentNode != null
				|| MaterialCommon.isCategoryContainMaterial(getContext(), categoryInfo.getId())) {//父类设置了属性或该分类下已关联商品时，属性不可编辑
			isEditable = false;
		}
	...
	private void doPropertyTableAction(final String rowId, String actionName,
		...
		if (PropertyActions.remove.name().equals(actionName)) {
			confirm("确认是否删除所选材料属性？", new Runnable() {
			...
			// 取得表格里每一行的数据
			String[] rowIds = propertiesTable.getAllRowId();
			List<MaterialsPropertyDefine> allPropertyElmentList = new ArrayList<MaterialsPropertyDefine>();
			...
			MaterialsPropertyDefine[] allPropertyElments = allPropertyElmentList.toArray(new MaterialsPropertyDefine[allPropertyElmentList.size()]);
			if (PropertyActions.up.name().equals(actionName)) {
				if (actionRowIndex <= 1) {
					alert("“单位”必须在一行，不能在移动了。");
				} else {
					MaterialsPropertyDefine tempElemnt = allPropertyElments[actionRowIndex];
					MaterialsPropertyDefine preElemnt = allPropertyElments[actionRowIndex - 1];
				...
				if (actionRowIndex < rowIds.length - 1) {
					MaterialsPropertyDefine tempElemnt = allPropertyElments[actionRowIndex];
					MaterialsPropertyDefine nextElemnt = allPropertyElments[actionRowIndex + 1];
...
//		}
		...
		if (null == propertiedParentNode) {
			List<MaterialsPropertyDefine> pdList = new ArrayList<MaterialsPropertyDefine>();
			for (int index = 0; index < ui_propertyIds.size(); index++) {
				String pdId = ui_propertyIds.get(index);
				MaterialsPropertyDefine pdTemp = ui_propertyDefines.get(pdId);
				if (pdTemp.getName() != null && !"".equals(pdTemp.getName())) {
					pdList.add(ui_propertyDefines.get(pdId));
				}
			}
			task.setPropertyDefines(pdList.toArray(new MaterialsPropertyDefine[pdList.size()]));
		}
		try {
...
//			if (!lastPropertyName.trim().equals("")) {
				for (String key : ui_propertyDefines.keySet()) {
					MaterialsPropertyDefine temp = ui_propertyDefines.get(key);
...
//			}
	...
	class PropertyContentProvider implements SEditContentProvider {
		...
		public PropertyContentProvider(MaterialsPropertyDefine[] propertyDefines,
				boolean editable) {
			if (propertyDefines == null || propertyDefines.length == 0) {
				this.propertyDefines = new MaterialsPropertyDefine[] { new PropertyDefineItem(
						GUID.randomID(), "单位",
						DEFAULT_UNITS) }; // TOOD:单位枚举
			} else {
		...
		public String getElementId(Object element) {
			return ((MaterialsPropertyDefine) element).getId().toString();
		}
		...
		public String getValue(Object element, String columnName) {
			...
			if (editable) {
				if (PropertyColumns.name.name()
						.equals(columnName)) {
					MaterialsPropertyDefine item = (MaterialsPropertyDefine)element;
					if(item.getName().equals("单位")) {
						return null;
					}
		...
		public String[] getActionIds(Object element) {
			MaterialsPropertyDefine value = (MaterialsPropertyDefine)element;
			...
			if (newElementId != null && newElementId.equals(value.getName())) {
				...
				if (propertyDefines.length > 1) {
					MaterialsPropertyDefine firstValue = propertyDefines[1];
					if (!value.equals(firstValue)) {
						actionIds.add(PropertyActions.up.name());
					}
				}
				if (propertyDefines.length > 2) {
					MaterialsPropertyDefine lastValue = propertyDefines[propertyDefines.length - 1];
		...
		public Object getNewElement() {
			if(propertiesTable.getAllRowId().length < 12) {
				...
				MaterialsPropertyDefine pd = new PropertyDefineItem(pdId, "", null);
		...
		public SNameValue[] getExtraData(Object element) {
			if(element instanceof PropertyDefineItem || element instanceof MaterialsPropertyDefine) {
				MaterialsPropertyDefine item = (MaterialsPropertyDefine)element;
				if(item.getName().equals("单位")) {
...
//			values = getSelectedPdOptionValues();
		...
		public String getValue(Object element, String columnName) {
			...
			if (editable) {
				if (OptionColumns.item.name().equals(
						columnName)) {
...
class PropertyDefineItem implements MaterialsPropertyDefine {
