			...
			public void actionPerformed(String rowId, String actionName, String actionValue) {
				if ((Action.Detail.name() + "1").equals(actionName)) {
					...
					if (sheetNo.startsWith("XSD")) {
						...
						PageControllerInstance pci = new PageControllerInstance("Psi_PruchaseReturnOrderDetailPages",
								info);
	...
	private void calTotalApplyAmount() {
		...
		if (null == selectedRowIds || selectedRowIds.length < 1) {
			...
			for (String rowId : selectedRowIds) {
				...
				double molingAmount = StringUtils.isEmpty(molingAmountStr) ? 0.0 : DoubleUtil
	...
	private class TableContentProvider implements SEditContentProvider {
		...
		public Object[] getElements(Context context, STableStatus tablestatus) {
			if (null == partnerInfo) {
				...
				// ReceiptType.getReceiptType(receiptType);
				GetReceiptingInventorySheetKey key = new GetReceiptingInventorySheetKey(partnerInfo.getId(),
						receiptType);
