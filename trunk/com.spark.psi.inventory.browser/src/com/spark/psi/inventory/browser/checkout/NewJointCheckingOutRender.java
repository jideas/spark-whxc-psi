...
import com.spark.psi.inventory.browser.checkout.NewJointCheckingOutProcessor.Column;
import com.spark.psi.publish.base.materials.entity.MaterialsItemInfo;

public class NewJointCheckingOutRender extends PSIListPageRender {

	@Override
	protected void afterFooterRender() {
		super.afterFooterRender();
		Button confirmBtn = new Button(footerRightArea, JWT.APPEARANCE3);
		confirmBtn.setID(NewJointCheckingOutProcessor.ID_Button_Confirm);
	...
	protected void afterTableRender() {
		...
		addGoodsBtn.setID(NewJointCheckingOutProcessor.ID_Button_AddGoods);
		...
		memoText.setID(NewJointCheckingOutProcessor.ID_Text_Memo);
	}
	...
	protected void beforeTableRender() {
		...
		storeList.setID(NewJointCheckingOutProcessor.ID_List_Store);
		...
		t.setID(NewJointCheckingOutProcessor.ID_Text_DeliveryPerson);
		new Label(tableHeaderArea).setText("    提货单位：");
		t = new Text(tableHeaderArea, JWT.APPEARANCE3);
		t.setID(NewJointCheckingOutProcessor.ID_Text_DeliveryUnit);

		new Label(tableHeaderArea).setText("    凭证号：");
		t = new Text(tableHeaderArea, JWT.APPEARANCE3);
		t.setID(NewJointCheckingOutProcessor.ID_Text_VoucherNumber);
	...
	public STableColumn[] getColumns() {
		...
		columns[0] = new STableColumn(NewJointCheckingOutProcessor.Column.code.name(), 100, JWT.LEFT,
				NewJointCheckingOutProcessor.Column.code.getTitle());
		columns[1] = new STableColumn(NewJointCheckingOutProcessor.Column.number.name(), 100, JWT.LEFT,
				NewJointCheckingOutProcessor.Column.number.getTitle());
		columns[2] = new STableColumn(NewJointCheckingOutProcessor.Column.name.name(), 100, JWT.LEFT,
				NewJointCheckingOutProcessor.Column.name.getTitle());
		columns[3] = new STableColumn(NewJointCheckingOutProcessor.Column.property.name(), 200, JWT.LEFT,
				NewJointCheckingOutProcessor.Column.property.getTitle());
		columns[4] = new STableColumn(NewJointCheckingOutProcessor.Column.unit.name(), 100, JWT.CENTER,
				NewJointCheckingOutProcessor.Column.unit.getTitle());
		columns[5] = new STableColumn(NewJointCheckingOutProcessor.Column.inventoryCount.name(), 100, JWT.RIGHT,
				NewJointCheckingOutProcessor.Column.inventoryCount.getTitle());
		columns[6] = new SNumberEditColumn(NewJointCheckingOutProcessor.Column.count.name(), 100, JWT.RIGHT,
				NewJointCheckingOutProcessor.Column.count.getTitle());
	...
	public String getText(Object element, int columnIndex) {
		...
		case 5:
			Inventory inventory = getContext().find(Inventory.class, GUID.valueOf(this.storeList.getText()),
					goodsItem.getItemData().getId());
			if (null != inventory) {
				return DoubleUtil.getRoundStr(inventory.getCount());
			}
			return "0.00";
		default:
			return null;
