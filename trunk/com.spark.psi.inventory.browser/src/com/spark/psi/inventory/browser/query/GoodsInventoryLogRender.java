...
import com.spark.psi.inventory.browser.query.GoodsInventoryLogProcessor.GoodsInventoryLogQueryListProcessor;
import com.spark.psi.publish.inventory.entity.InventoryLogItem;

public class GoodsInventoryLogRender extends GoodsCategoryFramePageRender {

	public final static class GoodsInventoryLogQueryListRender extends PSIGoodsListPageRender {
		...
		protected void afterFooterRender() {
			...
			// 
			new SDatePicker(headerLeftArea).setID(GoodsInventoryLogQueryListProcessor.ID_DATE_BEGIN);
			new Label(headerLeftArea).setText(" 到：");
			new SDatePicker(headerLeftArea).setID(GoodsInventoryLogQueryListProcessor.ID_DATE_End);
			new Button(headerLeftArea).setID(GoodsInventoryLogQueryListProcessor.ID_BUTTON_QueryButton);
			//
			new Label(headerLeftArea).setText("  记录数量：");
			//
			Label label = new Label(headerLeftArea);
			label.setID(GoodsInventoryLogQueryListProcessor.ID_LABEL_INVENTORYLOG_COUNT);
		...
		public STableColumn[] getColumns() {
			...
			// if (isStoreKeeper()) {
			// columns[8] = new STableColumn(
			// InventoryLogQueryProcessor.Columns.CheckedInCount
			// .name(), 100, JWT.RIGHT, "入库数量");
			// columns[9] = new STableColumn(
			// InventoryLogQueryProcessor.Columns.CheckedOutCount
			// .name(), 100, JWT.RIGHT, "出库数量");
			// } else {
			columns[8] = new STableColumn(InventoryLogQueryProcessor.Columns.CheckedInCount.name(), 100, JWT.RIGHT,
					"入库数量");
			columns[9] = new STableColumn(InventoryLogQueryProcessor.Columns.CheckedInAmount.name(), 100, JWT.RIGHT,
					"入库金额");
			columns[10] = new STableColumn(InventoryLogQueryProcessor.Columns.CheckedOutCount.name(), 100, JWT.RIGHT,
					"出库数量");
			columns[11] = new STableColumn(InventoryLogQueryProcessor.Columns.CheckedOutAmount.name(), 100, JWT.RIGHT,
					"出库金额");
			// }
		...
		// return getContext().find(Boolean.class, Auth.StoreKeeper)
		// && !getContext().find(Boolean.class, Auth.Boss)
		// && !getContext().find(Boolean.class,
		...
		public String getText(Object element, int columnIndex) {
			...
			// if (0 == item.getCheckedInCount() && 0 ==
			// item.getCheckedInAmount()) {
			...
			// if (0 == item.getCheckedOutCount() && 0 ==
			// item.getCheckedOutAmount()) {
