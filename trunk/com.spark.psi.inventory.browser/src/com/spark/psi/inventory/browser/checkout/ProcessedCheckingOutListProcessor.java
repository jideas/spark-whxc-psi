 ...
 * 已处理完成的出库单列表处理器
 */
public class ProcessedCheckingOutListProcessor extends PSIListPageProcessor<CheckoutSheetItem> {
	...
	public Object[] getElements(Context context, STableStatus tablestatus) {
		...
		List<CheckoutSheetItem> itemList = context.getList(CheckoutSheetItem.class, key);
	...
	public void actionPerformed(String rowId, String actionName, String actionValue) {
		if (ID_ACTION_EDIT.equals(actionName)) {
			if (CheckingOutType.Kit.equals(itemMap.get(rowId).getSheetType())) {
				PageController pc = new PageController(KitCheckOutSheetDetailProcessor.class, KitCheckOutSheetDetailRender.class);
			...
			} else if (CheckingOutType.Joint.equals(itemMap.get(rowId).getSheetType())||CheckingOutType.Gift.equals(itemMap.get(rowId).getSheetType())) {
				PageController pc = new PageController(JointCheckedOutDetailProcessor.class, JointCheckedOutDetailRender.class);
				PageControllerInstance pci = new PageControllerInstance(pc, rowId);
				MsgRequest request = new MsgRequest(pci, "出库单详情");
				request.setResponseHandler(new ResponseHandler() {
					public void handle(Object returnValue, Object returnValue2, Object returnValue3, Object returnValue4) {
				...
				CheckOutBaseInfo info = getContext().find(CheckOutBaseInfo.class, GUID.valueOf(rowId));
				PageController pc = new PageController(CheckedOutDetailProcessor.class, CheckedOutDetailRender.class);
