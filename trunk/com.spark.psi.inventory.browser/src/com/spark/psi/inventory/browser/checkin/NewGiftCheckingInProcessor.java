...
public class NewGiftCheckingInProcessor extends PSIListPageProcessor<CheckingInItem> implements IDataProcessPrompt {
	...
	private boolean doSave() {
		...
		final GiftCheckinTask task = new GiftCheckinTask();
		fillDataToTask(task);
	...
	private void fillDataToTask(GiftCheckinTask task) {
		...
		List<GiftCheckinTaskItem> items = new ArrayList<GiftCheckinTaskItem>();
		for (String rowId : rowIds) {
			String amount = table.getEditValue(rowId, Column.count.name())[0];
			items.add(new GiftCheckinTaskItem(GUID.valueOf(rowId), DoubleUtil.strToDouble(amount)));
		}
		task.setItems(items);
	}
