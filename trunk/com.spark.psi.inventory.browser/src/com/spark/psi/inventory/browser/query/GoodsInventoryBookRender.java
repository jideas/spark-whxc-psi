...
public class GoodsInventoryBookRender extends GoodsCategoryFramePageRender {
	...
	public final static class GoodsBookQueryListRender extends
		...
		protected void afterFooterRender() {
			...
			// .setID(InventoryBookQueryListProcessor.ID_COMBOLIST_STORE); 
			new SDatePicker(headerLeftArea)
					.setID(InventoryBookQueryListProcessor.ID_COMBOLIST_TERM1);
			new Label(headerLeftArea).setText("至");
			new SDatePicker(headerLeftArea)
					.setID(InventoryBookQueryListProcessor.ID_COMBOLIST_TERM2);
			...
			label
					.setID(InventoryBookQueryListProcessor.ID_LABEL_INVENTORYBOOK_COUNT);
		...
		public STableColumn[] getColumns() {
			...
			int index = 0;
			// 需要加SheetId 获取调拨单ID
			columns[index] = new STableColumn("goodsCode", 100, JWT.LEFT,
					"商品编码");
			...
			columns[index] = new STableColumn("goodsName", 100, JWT.LEFT,
					"商品名称");
			columns[index].setSortable(true);
			columns[index++].setGrab(true);
			columns[index] = new STableColumn("goodsAttr", 100, JWT.LEFT,
					"商品规格");
			columns[index].setSortable(true);
			columns[index++].setGrab(true);
			...
			columns[index++].setGrab(true);
			if (hasCountAuth()) {
				columns[index] = new STableColumn("count_begin", 90, JWT.RIGHT,
						"期初库存数量");
				columns[index].setSortable(true);
				columns[index++].setGrab(true);
			}
			if (hasAmountAuth()) {
				columns[index] = new STableColumn("amount_begin", 90,
						JWT.RIGHT, "期初库存金额");
				columns[index].setSortable(true);
				columns[index++].setGrab(true);
			}
			if (hasCountAuth()) {
				columns[index] = new STableColumn("instoCount", 70, JWT.RIGHT,
						"入库数量");
				columns[index].setSortable(true);
				columns[index++].setGrab(true);
			}
			if (hasAmountAuth()) {
				columns[index] = new STableColumn("instoAmount", 70, JWT.RIGHT,
						"入库金额");
				columns[index].setSortable(true);
				columns[index++].setGrab(true);
			}
			if (hasCountAuth()) {
				columns[index] = new STableColumn("outstoCount", 70, JWT.RIGHT,
						"出库数量");
				columns[index].setSortable(true);
				columns[index++].setGrab(true);
			}
			if (hasAmountAuth()) {
				columns[index] = new STableColumn("outstoAmount", 70,
						JWT.RIGHT, "出库金额");
				columns[index].setSortable(true);
				columns[index++].setGrab(true);
			}
			if (hasCountAuth()) {
				columns[index] = new STableColumn("count_end", 90, JWT.RIGHT,
						"期末库存数量");
				columns[index].setSortable(true);
				columns[index++].setGrab(true);
			}
			if (hasAmountAuth()) {
				columns[index] = new STableColumn("amount_end", 90, JWT.RIGHT,
						"期末库存金额");
				columns[index].setSortable(true);
				columns[index++].setGrab(true);
			}
		...
		public String getText(Object element, int columnIndex) {
			...
			if (columnIndex >= 0 && columnIndex < columns.length) {
				...
				if (fieldName.equals("goodsCode")) { // 编码/条码
					...
					// GoodsItem gi = this.getContext().find(GoodsItem.class,
					// book.getGoodsId());
					// if (null == gi) {
					// return null;
					// }
		...
		private boolean hasAmountAuth() {
			...
			// return getContext().find(Boolean.class,
			// Auth.SubFunction_InventoryQuery_Price, id);
		}
		...
		private boolean hasCountAuth() {
			...
			// GUID id = getContext().find(Login.class).getEmployeeId();
			// return getContext().find(Boolean.class,
			// Auth.SubFunction_InventoryQuery_Count, id);
		}
	}
}
