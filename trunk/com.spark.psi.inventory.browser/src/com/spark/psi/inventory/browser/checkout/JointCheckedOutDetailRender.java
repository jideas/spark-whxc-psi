 ...
 * 入库单明细列表视图
 */
public class JointCheckedOutDetailRender extends ExtendSimpleSheetPageRender {
	...
	public STableColumn[] getColumns() {
		...
		columns[0] = new STableColumn(JointCheckedOutDetailProcessor.Columns.GoodsCode.name(), 150, JWT.LEFT, "材料编号");
		columns[1] = new STableColumn(JointCheckedOutDetailProcessor.Columns.GoodsNo.name(), 150, JWT.LEFT, "材料条码");
		columns[2] = new STableColumn(JointCheckedOutDetailProcessor.Columns.GoodsName.name(), 100, JWT.LEFT, "材料名称");
		columns[2].setGrab(true);

		columns[3] = new STableColumn(JointCheckedOutDetailProcessor.Columns.GoodsProperties.name(), 100, JWT.LEFT, "材料规格");
		columns[3].setGrab(true);
		columns[4] = new STableColumn(JointCheckedOutDetailProcessor.Columns.GoodsUnit.name(), 120, JWT.CENTER, "单位");

		columns[5] = new STableColumn(JointCheckedOutDetailProcessor.Columns.CheckedCount.name(), 150, JWT.RIGHT, "出库数量");
	...
	protected void fillBaseInfoCellControl(Composite baseInfoArea, int row, int column) {
		if (0 == row) {
			if (0 == column) {
				...
				label.setID(JointCheckedOutDetailProcessor.ID_Label_Store);
			}
		}
		if (1 == row) {
			if (0 == column) {
				...
				t.setID(JointCheckedOutDetailProcessor.ID_Text_DeliveryPerson);
				new Label(baseInfoArea).setText("    提货单位：");
				t = new Label(baseInfoArea);
				t.setID(JointCheckedOutDetailProcessor.ID_Text_DeliveryUnit);

				new Label(baseInfoArea).setText("    凭证号：");
				t = new Label(baseInfoArea);
				t.setID(JointCheckedOutDetailProcessor.ID_Text_VoucherNumber);
	...
	protected void renderSheetButtonArea(Composite sheetButtonArea) {
		sheetButtonArea.setID(JointCheckedOutDetailProcessor.ID_FooterRightArea);
	}
