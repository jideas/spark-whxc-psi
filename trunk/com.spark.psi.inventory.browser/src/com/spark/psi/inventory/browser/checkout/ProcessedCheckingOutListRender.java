...
//		new Button(headerRightArea).setID(ProcessedCheckingOutListProcessor.ID_BUTTON_SEARCH);
	...
	public STableColumn[] getColumns() {
		...
		//需要加SheetId 获取调拨单ID
		columns[0] = new STableColumn(ProcessedCheckingOutListProcessor.Columns.LastCheckoutDate.name(), 120, JWT.CENTER, "出库日期");
		
		columns[1] = new STableColumn(ProcessedCheckingOutListProcessor.Columns.SheetNumber.name(), 120, JWT.CENTER, "出库单号");
		columns[1].setGrab(true);

		columns[2] = new STableColumn(ProcessedCheckingOutListProcessor.Columns.Type.name(), 120, JWT.CENTER, "出库类型");
		columns[2].setGrab(true);

		columns[3] =new STableColumn(ProcessedCheckingOutListProcessor.Columns.RelatedNumber.name(), 120, JWT.CENTER, "相关单据");
		columns[3].setGrab(true);		
		
		columns[4] = new STableColumn(ProcessedCheckingOutListProcessor.Columns.StoreName.name(), 120, JWT.CENTER, "仓库");
		columns[4].setGrab(true); 
		
		columns[5] = new STableColumn(ProcessedCheckingOutListProcessor.Columns.CheckoutEmployees.name(), 120, JWT.CENTER, "确认出库");
