...
import com.spark.psi.inventory.browser.checkin.NewAdjustCheckingInProcessor.Column;
...
public class NewAdjustCheckingInRender extends PSIListPageRender {

	@Override
	protected void afterFooterRender() {
		super.afterFooterRender();
		Button confirmBtn = new Button(footerRightArea, JWT.APPEARANCE3);
		confirmBtn.setID(NewAdjustCheckingInProcessor.ID_Button_Confirm);
	...
	protected void afterTableRender() {
		...
		addGoodsBtn.setID(NewAdjustCheckingInProcessor.ID_Button_AddGoods);
		...
		memoText.setID(NewAdjustCheckingInProcessor.ID_Text_Memo);
		...
		amountLabel.setID(NewAdjustCheckingInProcessor.ID_Label_Amount);
	...
	protected void beforeTableRender() {
		...
		providerText.setID(NewAdjustCheckingInProcessor.ID_Text_Provider);
		...
		storeList.setID(NewAdjustCheckingInProcessor.ID_List_Store);
	...
	public STableColumn[] getColumns() {
		...
		columns[0] = new STableColumn(NewAdjustCheckingInProcessor.Column.code.name(), 100, JWT.LEFT,
				NewAdjustCheckingInProcessor.Column.code.getTitle());
		columns[1] = new STableColumn(NewAdjustCheckingInProcessor.Column.number.name(), 100, JWT.LEFT,
				NewAdjustCheckingInProcessor.Column.code.getTitle());
		columns[2] = new STableColumn(NewAdjustCheckingInProcessor.Column.name.name(), 100, JWT.LEFT,
				NewAdjustCheckingInProcessor.Column.name.getTitle());
		columns[3] = new STableColumn(NewAdjustCheckingInProcessor.Column.property.name(), 200, JWT.LEFT,
				NewAdjustCheckingInProcessor.Column.property.getTitle());
		columns[4] = new STableColumn(NewAdjustCheckingInProcessor.Column.unit.name(), 100, JWT.CENTER,
				NewAdjustCheckingInProcessor.Column.unit.getTitle());
		columns[5] = new SNumberEditColumn(NewAdjustCheckingInProcessor.Column.amount.name(), 100, JWT.RIGHT,
				NewAdjustCheckingInProcessor.Column.amount.getTitle());
