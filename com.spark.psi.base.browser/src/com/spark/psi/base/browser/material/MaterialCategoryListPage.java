...
public class MaterialCategoryListPage extends Page {
	//
	...
	public MaterialCategoryListPage(Composite parent, final boolean canAdd) {
		super(parent);
		this.canAdd = canAdd;
		...
		this.getContext().regMessageListener(MaterialCategorySelectionMsg.class,
				new MessageListener<MaterialCategorySelectionMsg>() {
					public void onMessage(
						...
						} else if ((!MaterialCategoryListPage.this.canAdd && targetNode
	...
	private void reloadContent(CategoryNode[] showNodes) {
		...
		browser.addActionListener(new BrowserActionListener() {
			public void actionPerformed(BrowserActionEvent e) {
				getContext().broadcastMessage(
						new MaterialCategorySelectionMsg(GUID
...
//			"PSIBase.GoodsCategoryList.handleAddButtonKeyDown");
		...
		if(targetNodeId != null) { 
			MaterialsCategoryTree categoryTeee = getContext().find(MaterialsCategoryTree.class);
			...
			if (node != null) {
				...
				if((node.getChildren() == null || node.getChildren().length < 1)
						&& MaterialCommon.isCategoryContainMaterial(getContext(), targetNodeId)) {
	...
	private void handleAddCategory(String categoryName) {
		...
		// 验证商品分类名称是否重复
		if(MaterialCommon.isExistCategoryName(getContext(), targetNodeId, categoryName)) {
		...
		if (MaterialCommon.isExistCategoryCode(getContext(), targetNodeId, categoryNo.trim())) {
			new SMessageAlertWindow(true, " 编号不能重复！", null);
			return;
		}
		
		
		CreateMaterialsCategoryTask task = new CreateMaterialsCategoryTask(
		...
		getContext().broadcastMessage(
				new MaterialCategorySelectionMsg(targetNodeId));
