...
public class GoodsItemInventoryInfoPage extends Composite {
	private ThresholdControlType viewType;
	private GoodsInventoryScope viewScope;
	private GoodsItemData goodsItem;
	...
	public GoodsItemInventoryInfoPage(Composite parent,
			GoodsItemData goodsItem, GoodsInventoryScope viewScope,
...
//		glHeader.marginTop = 10;
	...
	private void fillContent() {
		// header
		new Label(headerArea).setText("商品属性："
				+ goodsItem.getPropertiesWithoutUnit());
...
//		tableStyle.setRowHeight(24);
		...
		itemTable.setContentProvider(new SEditContentProvider() {
			...
			public Object[] getElements(Context context, STableStatus tablestatus) {
				GoodsItemInfo goodsItemInfo = context.find(GoodsItemInfo.class, goodsItem.getId());
		...
		if(ThresholdControlType.AMOUNT.equals(viewType)) { // 
			if(GoodsInventoryScope.Total.equals(viewScope)) { // 金额总库存
				...
				if (storeAmountUpper != -1) {
					GoodsItemThresholdItem thresHoldItem = new GoodsItemThresholdItem(goodsItem.getId(), storeAmountUpper);
					UpdateGoodsItemThresholdTask totalTask = new UpdateGoodsItemThresholdTask(thresHoldItem);
					getContext().handle(totalTask);
				}
			} else { // 金额分库存
				List<GoodsItemThresholdItem> itemList = new ArrayList<GoodsItemThresholdItem>();
				for(int rowIndex = 0; rowIndex < rowIds.length - 1; rowIndex++) {
					...
					GoodsItemThresholdItem item = new GoodsItemThresholdItem(goodsItem.getId(), GUID.tryValueOf(rowIds[rowIndex]), storeAmountUpper);
					itemList.add(item);
				}
				if (itemList.size() > 0) {
					UpdateGoodsItemThresholdTask itemTask = new UpdateGoodsItemThresholdTask(itemList.toArray(new GoodsItemThresholdItem[itemList.size()]));
...
//				getContext().handle(totalTask);
				...
				for(int rowIndex = 0; rowIndex < rowIds.length - 1; rowIndex++) {
					...
					GoodsItemThresholdItem item = new GoodsItemThresholdItem(goodsItem.getId(), GUID.tryValueOf(rowIds[rowIndex]), storeUpper, storeLower);
					itemList.add(item);
				}
				if (itemList.size() > 0) {
					UpdateGoodsItemThresholdTask itemTask = new UpdateGoodsItemThresholdTask(itemList.toArray(new GoodsItemThresholdItem[itemList.size()]));
